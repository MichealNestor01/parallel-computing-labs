...
int main( int argc, char **argv )
{
	...
	int USE_POINT_TO_POINT = atoi(argv[1]);
	...
	if (USE_POINT_TO_POINT) {
		...
		// collective communication using mpi_scatter
		MPI_Scatter(
			globalData, localSize, MPI_INT, // sent from
			localData, localSize, MPI_INT,  // recieved to 
			0, MPI_COMM_WORLD		 // source rank 0
		);
	}
	...
	if (USE_POINT_TO_POINT) {
		...
		// collective communication using mpi_reduce
		MPI_Reduce(
			&count, &total, 1, MPI_INT, // sent from
			MPI_SUM, 0, MPI_COMM_WORLD  // recieved to 
		);
	}
